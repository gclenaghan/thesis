...
\begin{cor}
	Let $f : X \rightarrow B$ be a flat morphism from any scheme $X$ to a smooth, 1-dimensional scheme $B$ over a field of characteristic 0.
	Then there is a spectral sequence computing the homology of $\DB_{X/B}^p$:
	\[
...
\begin{proof}
	...
	Construct the injective Cartan-Eilenberg resolution of the resolution from theorem \ref{thm:locfreereldiffs}, apply the pushforward functor and compute the spectral sequence, obtaining
	\[
		E_1^{ij} = \bR^i \pi_{\bullet *} (f_\bullet^* \omega_B^{\otimes(p-j)} \otimes \Omega_{X_\bullet}^j) \Rightarrow \bR^{i+j} \pi_{\bullet *} \Omega_{X/B}^p
	\]
	This is part of the Grothendieck spectral sequence for $\bR \pi_{\bullet *}$ and $\id$, we are just stopping at $E_1$.
