...
\begin{lemma}
	Let $X_\bullet$ be an $I$-scheme and $\EE_{\alpha \bullet}$ be as defined above.
	Then for any $F_\bullet \in D(\Sh(X_\bullet))$,
	\[
		\bR \cHom (\EE_{\alpha \bullet}, F_\bullet)_\beta = \begin{cases}
														F_\beta & \text{if } \beta \in \alpha/I \\
														\bR X_{\phi *} F_\alpha & \text{if } \phi : \alpha \rightarrow \beta \\
														0 & \text{otherwise.}
														\end{cases}
	\]
\end{lemma}
