...
\begin{definition}[10.71 of \cite{Kollar2013}]
	A {\it diagram of schemes over} $I$ (or just an $I$-{\it scheme}) is a contravariant functor from a thin, finite category without loops (we'll call such a category a \textit{diagram category}) $I$ to the category of schemes.
...
\begin{definition}[\cite{Guillen1988} I 6]
	Let $F_\bullet : \Box_n \rightarrow C(A)$ be a diagram in the category of complexes in an abelian category $A$
	, where the differential of $F_\alpha$ is $d_\alpha$.
	Define the {\it simple complex} of $S(F_\bullet)$ to be the object in $C(A)$ with
	\[
	S(F_\bullet)^p = \bigoplus_\alpha F_\alpha^{1+p-|\alpha|}
	\] 
	and such that the differential on each $F_\alpha^{1+p-|\alpha|}$ is
	\[
...
\begin{prop}[\cite{Guillen1988} I 6.3 ]
	Let $A$ be an abelian category, then the simple complex is a functor $S:\Box_n-C(A) \rightarrow C(A)$ which takes quasi-isomorphisms to quasi-isomorphisms.
\end{prop}
\begin{proof}
