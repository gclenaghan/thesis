...
\begin{defprop}[\cite{Lipman2009} II 6.7]
	Let $X_\bullet$ be an $I$ scheme, with $\alpha \rightarrow \beta$ a morphism in $I$. Define the inclusion $j_{\alpha \beta} : (X_\beta \rightarrow X_\alpha) \rightarrow X_\bullet$ of diagrams of schemes.
...
\begin{theorem}
	...
	Then, for $2 \leq i \leq \dim X - 1$, the exceptional locus $Y_p$ satisfies
	\[
		H^i (Y_p, \DB_{Y_p}^0) = 0,
	\]
...
\begin{proof}
	...
	Since $X$ is DB, Kodaira vanishing holds (see section 6 of \cite{Kovacs2010}) and $H^i(X, \omega_X^\bullet \otimes \LL) = 0$ for $i > -\dim X$, or via corollary \ref{cor:dualDBdef} $H^i(X, \db_X \otimes \LL) = 0$.
\end{proof}
