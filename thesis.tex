...
\section{Diagrams of Schemes}
\begin{definition}[10.71 of \cite{Kollar2013}]
	A {\it diagram of schemes over} $I$ (or just an $I$-{\it scheme}) is a contravariant functor from a thin, finite category $I$ to the category of schemes.
	A morphism between an $I$-scheme $\Phi : I^{op} \rightarrow \Sch$ and a $J$-scheme $\Psi : J^{op} \rightarrow \Sch$ is given by a functor $\Gamma : I^{op} \rightarrow J^{op}$ and a natural transformation $\eta : \Phi \rightarrow \Psi \circ \Gamma$.
	In simpler terms, a diagram of schemes is a finite collection of schemes with morphisms between them, and a morphism of diagrams of schemes is a collection of morphisms between the elements of the two collections where all the maps involved are compatible.
	
	We'll call $I$ the {\it shape} of an $I$-scheme.
	Call a morphism of diagrams of schemes {\it shape-preserving} if $\Gamma$ is an equivalence, and {\it shape-embedding} if $\Gamma$ is fully-faithful.
	Finally, we'll usually denote a diagram of schemes as $X_\bullet$, where the shape $I$ and functor $\Phi$ are implicit.
	For $\alpha \in I$, denote $\Phi(\alpha) = X_\alpha$, and for morphisms $\phi$ in $I$ denote $\Phi(\phi) = X_\phi$.
\end{definition}
...
\begin{definition}[10.84 of \cite{Kollar2013}]
	...
	Similarly, one can construct the derived category of these sheaves, which are collections of objects in the derived category over each of the $X_\alpha$, and maps $F_\beta \rightarrow \bR f_* F_\alpha$.
\end{definition}
...
Next, it turns out taking the limit over a $\Box_n$ diagram has a particularly nice form.
For notational convenience, we'll consider $\Box_n$ as a subset of $\ZZ^n$, where each component is either 0 or 1.
...
\begin{definition}
	Let $\Phi : \Box_n \rightarrow C(A)$ be a diagram in the category of complexes of objects in an abelian category $A$, where the differential of $\Phi(\alpha)$ is $d_\alpha$.
	Define the {\it staggered complex} of $\Phi$ to be the object $K \in C(A)$ with
	\[
	K^p = \bigoplus_\alpha \Phi(\alpha)^{1+p-|\alpha|}
	\] 
	and such that the differential on each $\Phi(\alpha)^{1+p-|\alpha|}$ is
	\[
	d = (\bigoplus_{\{i : \alpha_i = 0\}} (-1)^{\sum_{j < i} \alpha_j} \Phi(\phi_{(\alpha, i)})) \oplus (-1)^{|\alpha|} d_\alpha.
	\]
	One can make a similar construction for $K(A)$ or $D(A)$.
\end{definition}

The name is meant to be suggestive: this complex is obtained by combining all the complexes, staggering them to the right by the value of $|\alpha|$.
...
\end{tikzcd}
\]
Then to obtain the staggered complex, we arrange the complexes, written out, as
\[
\begin{tikzcd}
...
\begin{prop}
	...
	Then $\bR f_{\bullet *} F_\bullet$ is naturally quasi-isomorphic to the staggered complex formed by the $\Box_n$ diagram of $\bR f_{\alpha *} F_\alpha$ for each $\alpha \in \Box_n$.
\end{prop}
\begin{proof}
	This is I 6.6 of \cite{Guillen1988} in the special case of $I = \underline{1}$, and $K_\bullet$ the functor sending 0 to $\Box_n$.
\end{proof}

There is not yet a notion of global sections as with a sheaf on a scheme, so we make the following definition:
...
\begin{theorem}
	For any $X_\bullet$ an $I$-scheme over a field $k$, and $\alpha \in I$, there is a natural isomorphism
	\[
		(\omega_{X_\bullet}^\bullet)_\alpha = \bR \varprojlim_{I} \hat{\omega}^\alpha_\bullet
	\]
\end{theorem}
\begin{proof}
	For any $F \in D(X_\alpha)$, we have 
	\begin{align*}
	\Hom(F, (\omega_{X_\bullet}^\bullet)_\alpha) &= \Hom(j_{\alpha !} F_\bullet, \omega_{X_\bullet}^\bullet) \\
	&= \bR \Gamma(j_{\alpha !} F_\bullet)^* \\
	&= (\bR \varprojlim_{\beta \in I} \bR \Gamma (j_{\alpha !} F_\beta))^* \\
	\end{align*}
	
	For $\beta \in \alpha / I$, $j_{\alpha !} F_\beta = \bL X_{\phi_{\beta \alpha}}^* F$, so
	\begin{align*}
		\bR \Gamma(j_{\alpha !} F_\beta)^*  &= \Hom(\bL X_{\phi_{\beta \alpha}}^* F, \omega_{X_\alpha}^\bullet) \\
		&= \Hom(F, \bR X_{\phi_{\beta \alpha} *} \omega_{X_\alpha}^\bullet) \\
		&= \Hom(F, \hat{\omega}^\alpha_\beta)
	\end{align*}
	similarly, if $\beta \not \in \alpha / I$, both $\bR \Gamma(j_{\alpha !} F_\beta)^*$ and $\Hom(F, \hat{\omega}^\alpha_\beta)$ are zero.
	
	Thus, continuing from above,
	\begin{align*}
		\Hom(F, (\omega_{X_\bullet}^\bullet)_\alpha) &= \bR \varprojlim_{\beta \in I} \Hom(F, \hat{\omega}^\alpha_\beta) \\
		&= \Hom(F, \bR \varprojlim_{I} \hat{\omega}^\alpha_\bullet).
	\end{align*}
\end{proof}
