...
\section{Motivation}
Cubic hyperresolutions are an attempt to resolve the singularities of a variety while preserving the topological information of the original.
...
\begin{example}
	...
	If $\LL$ is an ample line bundle on $X$, then for $2 \leq i \leq \dim Y$, $\HH^{-i}(X, \LL \otimes \db_X)^* = H^{i-1}(Y, \OO_Y)$, in particular this is not zero if $Y$ is, say, a ruled surface over a curve of genus $g \geq 1$.
...
\begin{theorem}
	Let $X$ be a proper, Cohen-Macaulay $\CC$-scheme with Du Bois singularities, $Y$ smooth, with $f : Y \rightarrow X$ a proper morphism which is as isomorphism outside of $p \in X$.
	Then, for $0 < i < \dim X - 1$, the exceptional locus $Y_p$ satisfies
	\[
		H^i (Y_p, \DB_{Y_p}^0) = 0,
	\]
	in particular if $Y_p$ has Du Bois singularities, $H^i(Y_p, \OO_{Y_p}) = 0$.
	In addition, if $\dim X > 1$, then $H^0(Y_p, \DB_{Y_p}) = H^0(Y_p, \CC)= \CC$ and in particular $Y_p$ is connected.
\end{theorem}
\begin{proof}
	First, we set up the notation: let $n = \dim X$.
	By construction, $X$ has the 2-resolution
	\[
	\begin{tikzcd}
	...
	Let $\LL$ be any big and nef line bundle on $X$.
	Since $X$ is DB, Kodaira vanishing holds (see section 6 of \cite{Kovacs2010}) and $H^i(X, \omega_X^\bullet \otimes \LL) = 0$ for $i > -n$, so via corollary \ref{cor:dualDBdef} $H^i(X, \db_X \otimes \LL) = 0$.
	Using the projection formula and Grothendieck duality, this corresponds to $H^i(X_\bullet, \pi_\bullet^* \LL^{-1}) = 0$ for $i < n$, but this is $H^i(Y_\bullet, \bR g_{\bullet *} \pi_\bullet^* \LL^{-1})$, so we can first compute this sheaf, then use that to find the cohomology.
	...
	Let $\FF_\bullet$ denote this complex on $Y_\bullet$, and we now know $H^i(Y_\bullet, \FF_\bullet) = 0$ for $i < n$.
	
	To compute this, we use the spectral sequence $E_1^{pq} = \bigoplus_{|\alpha| = p+1} H^q(Y_\alpha, \FF_\alpha)$.
	Using Grothendieck vanishing and the fact that $H^i(Y, f^* \LL^{-1}) = 0$ for $i < n$, the spectral sequence becomes
	\[
	...
	Thus for $0 < i < n-2$, $H^i(Y_p, \DB_{Y_p}^0) = H^{i+1}(X_\bullet, \pi_\bullet^* \LL^{-1}) = 0$, and $H^0(Y_p, \DB_{Y_p}^0)/\CC = H^1(X_\bullet, \pi_\bullet^* \LL^{-1}) = 0$ so $H^0(Y_p, \DB_{Y_p}^0) = \CC$.
	
	Using \ref{thm:omnibus} (\ref{thm:omnibus:specseq}), $H^0(Y_p, \DB_{Y_p}) = H^0(Y_p, \CC)$ so the last part follows.
\end{proof}
