...
These topologically differ, as the cylinder has a non-trivial $\Ho^1 (\tilde{X}, \CC)$, while $\Ho^1 (X, \CC) = 0$, pushing the de Rham complex to $X$ naively would lose the property that $\Ho^i ( \tilde{X}, \CC) \cong \HH^i (\tilde{X}, \Omega_{\tilde{X}}^\bullet)$.
...
Since the original definition, several implementations of the hyperresolution have been developed, each resulting in the same Du Bois complex. The most tractable definition is the cubical hyperresolution which uses diagrams of schemes, and such diagrams are the main topic of this dissertation.

\section{Outline}
Chapter \ref{chp:diagrams} serves as a mostly self-contained introduction to diagrams and develops the tools we will use to study them. The main reference for diagrams of schemes is Guilli\'en, et. al \cite{Guillen1988}, and a restricted category of diagrams--only shape-preserving maps (defined in \ref{def:diagram}) are allowed--is studied in Lipman and Hashimoto \cite{Lipman2009}; this chapter builds on the framework of these, compiling results and then adding what is necessary for the following chapters.
...
\begin{definition}
	...
	\]
	where $(\beta, \phi)$ ranges over $\beta \in I^{op}$ and $\phi : \Phi(\beta) \rightarrow \alpha$ a morphism in $J^{op}$.
...
\begin{definition}[{\cite[10.76]{Kollar2013}}]
...
\begin{cor}
	...
	Furthermore when the adjoint is taken with $\bR f_{\bullet *}$ a functor on $D_{lqc}(X_\bullet)$, $f_\bullet^!$ maps $D_{lqc}^+(X_\bullet)$ to $D_{lqc}^+(X_\bullet)$.
\end{cor}
\begin{proof}
	...
	The last statement can be shown by the boundedness of the pushforward in Theorem \ref{thm:pushforwardbounds} and using \cite[Lemma 4.1.8]{Lipman2009}.
\end{proof}
...
\begin{theorem}
	...
	Let $X$ be a proper, reduced $\CC$-scheme of dimension $n$, $Y$ smooth, with $f:Y \rightarrow X$ a proper morphism which is an isomorphism outside of a closed $p \in X$. Then for $0 \leq i \leq n$ and any big and nef line bundle $\LL$ on $X$, there are isomorphisms
	\[
		H^i(X, \LL^{-1} \otimes \DB_X^0) = \begin{cases}
		0 & \text{if } i = 0 \\
		H^0(Y_p, \CC)/\CC & \text{if } i = 1 \\
		H^{i-1}(Y_p, \DB_{Y_p}^0) & \text{if } 2 \leq i \leq n-1 \\
		H^n(Y, f^* \LL^{-1}) \oplus H^{n-1}(Y_p, \DB_{Y_p}^0) & \text{if } i = n
		\end{cases}
	\]
	where $Y_p$ is the fiber of $f$ over $p$.
\end{theorem}
\begin{proof}
...
\begin{cor}
	...
	Let $X$ be a proper, reduced, Cohen-Macaulay $\CC$-scheme of dimension $n$ with Du Bois singularities, $Y$ smooth, with $f : Y \rightarrow X$ a proper morphism which is as isomorphism outside of a closed $p \in X$.
	Then, for $0 \leq i \leq n - 2$, the exceptional locus $Y_p$ satisfies
	\[
		H^i (Y_p, \DB_{Y_p}^0) = \begin{cases}
		\CC & \text{if } i = 0 \\
		0 & \text{if } 1 \leq i \leq n - 2.
		\end{cases}
	\]
	In particular, $Y_p$ is connected and if $Y_p$ has Du Bois singularities $H^i(Y_p, \OO_{Y_p}) = 0$ for ${0 < i < n - 1}$.
\end{cor}
\begin{proof}
	Under these hypotheses, for any big and nef $\LL$ on $X$, $H^i(X, \LL^{-1} \otimes \DB_X^0) = H^i(X, \LL^{-1}) = 0$ for $i < n$ since Kodaira vanishing holds for $X$.
