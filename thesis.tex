...
The main reference for diagrams of schemes is Guilli\'en, et. al \cite{Guillen1988}, and a restricted category of diagrams--only shape-preserving maps (defined in \ref{def:diagram}) are allowed--is studied in Lipman and Hashimoto \cite{Lipman2009}.
...
\begin{theorem}[Corollary \ref{cor:cohoofcontraction}]
	...
	\end{cases}
	\]
	In particular, $Y_p$ is connected (a fact already implied by Zariski's Main Theorem) and if $Y_p$ has Du Bois singularities $H^i(Y_p, \OO_{Y_p}) = 0$ for $0 < i < n - 1$.
\end{theorem}
